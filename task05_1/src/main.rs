...
fn eval(prog: &mut Vec<i32>) -> Result<()> {
    let mut ip: usize = 0;
    while ip < prog.len() {
        ip = eval_cmd(ip, prog)?;
        if ip == 0 {
...
/// Returns next ip
fn eval_cmd(ip: usize, prog: &mut Vec<i32>) -> Result<usize> {
    ...
    match cmd {
        ...
        Command::Write => {
            ...
            println!("Output: {}", get_param_value(ip + 1, params[0], prog)?);
        }
